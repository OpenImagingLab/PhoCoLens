...
flags.DEFINE_string('image_folder', '', 'Path to folder containing images.')
...
def main(_):
    ...
    # 获取文件夹中所有JPEG图像的路径
    image_paths = glob.glob(os.path.join(FLAGS.image_folder, '*.png')) + glob.glob(os.path.join(FLAGS.image_folder, '*.jpg'))
    ...
    for image_path in image_paths:
        ...
        # 运行模型
        pred_mos = run_model_single_image(model_config, FLAGS.num_classes, pp_config, params, image_path)

        # 添加预测结果到列表中
        scores.append(pred_mos)
    
    # 计算平均分数
    if scores:
        average_score = np.mean(scores)
        print('============== Average Predicted MOS:', average_score)
    else:
