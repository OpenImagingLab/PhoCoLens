            ...
            for j in range(num_steps):
                ...
                if operation.guidance_3:
                    ...
                    if operation.original_guidance:
                        ...
                        t_in = torch.cat([ts] * 2)
                        c_in = torch.cat([unconditional_conditioning, cond])
                        struct_c_t = self.model.module.structcond_stage_model(torch.cat([struct_cond] * 2), t_in)
                        e_t_uncond, e_t = self.model.module.apply_model(x_in, t_in, c_in, struct_c_t).chunk(2)
                        e_t = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
                        # del x_in
                    else:
                        struct_c_t = self.model.module.structcond_stage_model(struct_cond, ts)
                        e_t = self.model.module.apply_model(img_in, ts, cond, struct_c_t)
